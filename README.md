...
- <a href="https://www.codecogs.com/eqnedit.php?latex=\sum_{k}^{K}z^k_i&space;=&space;\sum_{k}^{K}\sum_{i}^{N}&space;x^k_{0i}&space;=&space;\sum_{i=1}^{N}y_i&space;=&space;\sum_k^K&space;v_k&space;\ge&space;QTD_{MIN_{veiculos}}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\sum_{k}^{K}z^k_i&space;=&space;\sum_{k}^{K}\sum_{i}^{N}&space;x^k_{0i}&space;=&space;\sum_{i=1}^{N}y_i&space;=&space;\sum_k^K&space;v_k&space;\ge&space;QTD_{MIN_{veiculos}}" title="\sum_{k}^{K}z^k_i = \sum_{k}^{K}\sum_{i}^{N} x^k_{0i} = \sum_{i=1}^{N}y_i = \sum_k^K v_k \ge QTD_{MIN_{veiculos}}" /></a>
